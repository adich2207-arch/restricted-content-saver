...
async def process_and_upload_link(userbot, user_id, msg_id, link, retry_count, message):
    try:
        ...
        # Stop userbot after use to free memory (only if it's a per-user session)
        try:
...
async def single_link(_, message):
    ...
    if userbot is None:
        await msg.edit_text(
            ...
            "To save content from restricted channels, you need to login first.\n\n"
...
async def initialize_userbot(user_id):
    ...
    if data and data.get("session"):
        try:
            ...
            userbot = Client(
                f"userbot_{user_id}",  # unique session name per user
                api_id=API_ID,
                api_hash=API_HASH,
                device_model=device,
            ...
            await app.send_message(user_id, f"⚠️ Login session expired. Please /login again.\n\nError: `{e}`")
            return None
    else:
        if DEFAULT_SESSION:
            return userrbot
...
async def batch_link(_, message):
    ...
    join_button = InlineKeyboardButton("👨‍💻 Developed by @Mr_1X8", url="https://t.me/Mr_1X8")

...
async def subscribe(app, message):
    ...
    if not update_channel:
        return 0  # No force-subscribe configured, allow all users
    try:
        url = await gen_link(app, update_channel)
        if not url:
            return 0  # Can't generate link, skip force-subscribe silently
        try:
            ...
            await message.reply_photo(
                photo="https://cashbackpower.site/uploads/img_1766639922_9432.jpg",
                caption=caption,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{url}")]])
            )
            return 1
        except Exception:
            return 0  # Can't check membership, allow user through
    except Exception:
        return 0  # Any other error, allow user through
